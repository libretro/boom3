...
| `HDR Scene Precision (Restart Required)` | Scene buffer depth in 30-bit HDR mode. `10-bit` quantizes each additive light pass to 10-bit steps with one stop of fold headroom. `FP16 (Half-Float)` removes per-pass quantization and the accumulation ceiling entirely: light sums build unbounded and roll off only at output. Double scene-buffer bandwidth. |
