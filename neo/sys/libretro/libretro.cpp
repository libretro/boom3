...
/* The full-screen triangle both composite passes draw.  At file scope
 * because two passes use it now, and a pass that binds its program
 * without binding these draws nothing at all. */
...
static void hdr_present( GLuint dstFbo ) {
	...
	*/
	...
	glVertexAttribPointer( 0, 2, GL_FLOAT, GL_FALSE, 0, hdr_tri_verts );
...
static void hdr_encode_present( GLuint dstFbo ) {
	...
	/* The same state hdr_present sets before its own draw.  Binding the
	 * program without this is what made the first version of this pass
	 * produce a black frame: the 2D pass had just left depth, stencil
	 * and scissor enabled, and no vertex array was bound, so the
	 * full-screen triangle had no positions and every fragment that did
	 * reach the target was rejected. */
	glDisable( GL_DEPTH_TEST );
	glDisable( GL_STENCIL_TEST );
	glDisable( GL_BLEND );
	glDisable( GL_CULL_FACE );
